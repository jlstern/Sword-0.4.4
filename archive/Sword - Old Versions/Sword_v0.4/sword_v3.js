...
function beginTurnRotation(){
	if(checkIfAllEnemiesDead() === 0){
		...
		clearInterval(idleEnemy);
		currentTurn = 0;
		encounterNumber += 1;
		distanceToEncounter = 700 + randomNumber();
		setEnemyTeam();
...
function enemyIdleAnimation(){
	var f = 0;
	var fInterval = 0;
	idleEnemy = setInterval(function(){
		enemyGetFrame(f);
		fInterval += 1;
		if(fInterval % 20 === 0){
			f += 1;
...
// dragImage(img, sourcex, sourcey, sourcewidth, sourceheight, destx, desty, destwidth, destheight)
		if(enemyTeam[i].hpCurrent > 0){
			ctx.drawImage(enemyFrame[enemyTeam[i].index], frame * 200, 0, 200, 200, 180 + i * 130 + distanceToEncounter, 0, 200, 200);
		}
	}
}
...
function popAlert(damage, target, isPlayerAttack){
	...
	popAlertAnimate[targetMod] = setInterval(function(){
		...
		ctx.fillText("-" + damage, 65 + isPlayerAttack * 240 + target * 130, counter);
		counter -= 1;
		if(counter === 20){
			ctx.clearRect(0, 0, 800, 100);
			clearInterval(popAlertAnimate[targetMod]);
